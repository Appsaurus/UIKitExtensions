//
//  UIImage+ColorProminence.swift
//  AppsaurusUIKit
//
//  Created by Brian Strobach on 3/8/18.
//	Original implementation: https://github.com/jathu/UIImageColors

import Foundation
import UIKit
import DinoDNA

public struct UIImageColorScheme {
...
public struct UIImageColorsCounter {
	...
	public static var sortedColorComparator: Comparator {
		return { (main, other) -> ComparisonResult in
...
extension UIImage {
	...
	fileprivate func getColorDominanceData(quality: UIImageColorsQuality = .high) -> (NSMutableArray, NSCountedSet) {
		...
		sortedColors.sort(comparator: UIImageColorsCounter.sortedColorComparator)
		return (sortedColors, imageColors)
	}

	public func getColorDominance(quality: UIImageColorsQuality = .high, _ completion: @escaping ([UIImageColorsCounter]) -> Void) {
		completeOnMainThread(self.getColorDominance(quality: quality), completion)
	}
	public func getColorDominance(quality: UIImageColorsQuality = .high) -> [UIImageColorsCounter]{
		return getColorDominanceData(quality: quality).0.compactMap({return $0 as? UIImageColorsCounter})
	}

	public func getColors(quality: UIImageColorsQuality = .high, _ completion: @escaping ([UIColor]) -> Void) {
		completeOnMainThread(self.getColors(quality: quality), completion)
	}

	public func getColors(quality: UIImageColorsQuality = .high) -> [UIColor]{
		return getColorDominanceData(quality: quality).0.compactMap({return ($0 as? UIImageColorsCounter)?.color.uicolor})
	}

	public func getColorScheme(quality: UIImageColorsQuality = .high, _ completion: @escaping (UIImageColorScheme) -> Void) {
		completeOnMainThread(self.getColorScheme(quality: quality), completion)
	}

	public func getColorScheme(quality: UIImageColorsQuality = .high) -> UIImageColorScheme {
		var proposed: [Double] = [-1,-1,-1,-1]
		let colorDominanceData = getColorDominanceData()
		var sortedColors: NSMutableArray = colorDominanceData.0
		let imageColors: NSCountedSet = colorDominanceData.1
